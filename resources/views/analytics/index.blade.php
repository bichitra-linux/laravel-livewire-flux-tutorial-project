        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-zinc-600 dark:text-zinc-400 mt-2">Comprehensive insights into your blog's performance</p>
                </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ number_format($stats['total_views']) }}</p>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ number_format($stats['unique_visitors']) }}</p>
                        <p class="text-xs text-green-600 dark:text-green-400 mt-1">
                            {{ $stats['total_views'] > 0 ? round(($stats['unique_visitors'] / $stats['total_views']) * 100) : 0 }}% conversion
                        </p>
                    </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ $stats['avg_time_on_site'] }}<span class="text-lg">m</span></p>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ $stats['bounce_rate'] }}<span class="text-lg">%</span></p>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 shadow-lg">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-zinc-900 dark:text-white">{{ number_format($stats['new_visitors']) }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2 bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-700 px-3 py-1 rounded-full">Last 30 Days</span>
                </div>
                    ...
                    <div class="space-y-3">
                        @foreach($topPosts as $index => $post)
                            <div class="flex items-start gap-4 p-4 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors group">
                                {{-- Rank Badge --}}
                                <div class="shrink-0">
                                    <div class="w-10 h-10 rounded-lg {{ $index === 0 ? 'bg-linear-to-br from-yellow-400 to-orange-500' : ($index === 1 ? 'bg-linear-to-br from-gray-300 to-gray-400' : ($index === 2 ? 'bg-linear-to-br from-orange-400 to-yellow-600' : 'bg-zinc-200 dark:bg-zinc-700')) }} flex items-center justify-center font-bold text-white shadow-lg">
                                        {{ $index + 1 }}
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    <a href="{{ route('posts.show', $post->slug) }}" 
                                       class="text-zinc-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 font-semibold line-clamp-1 transition-colors">
                                        {{ $post->title }}
                                    </a>
                                ...
                                <div class="shrink-0 text-right">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-2xl font-bold text-zinc-900 dark:text-white">{{ number_format($post->view_total) }}</span>
                                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 shadow-lg">
                    ...
                    @if($deviceStats->isNotEmpty())
                        <canvas id="deviceChart" class="mb-4"></canvas>
                        ...
                        <div class="space-y-2 mt-4">
                            @foreach($deviceStats as $device => $count)
                                ...
                                <div class="flex items-center justify-between p-3 rounded-lg {{ $colors['bg'] }}">
                                    <span class="text-sm font-medium {{ $colors['text'] }} capitalize">{{ $device }}</span>
                                    <div class="flex items-center gap-2">
                                        <span class="text-sm font-bold {{ $colors['text'] }}">{{ number_format($percentage, 1) }}%</span>
                                        <span class="text-xs text-zinc-500 dark:text-zinc-400">({{ number_format($count) }})</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        @foreach($browserStats as $browser => $count)
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-sm font-medium text-zinc-900 dark:text-white capitalize">{{ $browser }}</span>
                                    <span class="text-xs text-zinc-500 dark:text-zinc-400">{{ number_format($percentage, 1) }}%</span>
                                </div>
                                <div class="w-full h-2 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                                    <div class="h-full bg-linear-to-r from-teal-500 to-teal-600 rounded-full" style="width: {{ $percentage }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
