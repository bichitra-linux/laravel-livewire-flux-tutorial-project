<div> {{-- SINGLE ROOT for Livewire component --}}
    ...
    <form action="{{ route('newsletter.subscribe') }}" method="POST" class="space-y-4">
        ...
        <input 
            ...
            required 
            value="{{ old('email') }}"
            class="w-full px-3 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 @error('email') @enderror" />
        @error('email')
