        ...
        <div class="max-w-7xl mx-auto px-6 py-6">
            ...
            <div class="grid lg:grid-cols-12 gap-8 mb-12">
                        ...
                        <article class="relative rounded-xl overflow-hidden shadow-xl bg-gradient-to-r from-white to-gray-100 dark:from-gray-800 dark:to-gray-700 transform hover:scale-105 transition-transform duration-300">
                            <div class="lg:flex">
                                <div class="lg:w-1/2 bg-cover bg-center min-h-[300px] lg:min-h-[400px]"
                                    style="background-image: url('{{ $featured->image ?? asset('images/placeholder.jpg') }}');">
                                </div>
                                <div class="p-8 lg:w-1/2 flex flex-col justify-center">
                                    ...
                                    <h2 class="text-3xl lg:text-4xl font-extrabold text-gray-900 dark:text-white mb-4 leading-tight">
                                        <a href="{{ route('posts.show', $featured) }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200">{{ $featured->title }}</a>
                                    </h2>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <a href="{{ route('posts.show', $featured) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full font-semibold shadow-md transition-all duration-200">Read More</a>
                                        <a href="{{ route('posts.index') }}" class="text-blue-600 dark:text-blue-400 hover:underline font-medium">View All Posts</a>
                                    </div>
                                </div>
                            </div>
                        </article>
                ...
                <aside class="lg:col-span-4 space-y-6">
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
                        ...
                        <ul class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
                            @forelse($posts->take(5) as $p)
                                <li>
                                    <a href="{{ route('posts.show', $p) }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 block">
                                        <div class="flex items-start gap-3">
                                            <div class="flex-1">
                                                <div class="font-medium">{{ $p->title }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                                    {{ optional($p->created_at)->diffForHumans() }}</div>
                                            </div>
                                        </div>
                                    </a>
                                </li>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
                        <h3 class="font-bold text-lg text-gray-900 dark:text-white mb-4 flex items-center gap-2"><svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path>
                            </svg>Topics</h3>
                        <div class="flex flex-wrap gap-2">
                            <a href="#"
                                class="text-xs px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors duration-200">World</a>
                            <a href="#"
                                class="text-xs px-3 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded-full hover:bg-green-200 dark:hover:bg-green-800 transition-colors duration-200">Technology</a>
                            <a href="#"
                                class="text-xs px-3 py-1 bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 rounded-full hover:bg-purple-200 dark:hover:bg-purple-800 transition-colors duration-200">Business</a>
                            <a href="#"
                                class="text-xs px-3 py-1 bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200 rounded-full hover:bg-pink-200 dark:hover:bg-pink-800 transition-colors duration-200">Lifestyle</a>
                            <a href="#"
                                class="text-xs px-3 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded-full hover:bg-yellow-200 dark:hover:bg-yellow-800 transition-colors duration-200">Culture</a>
                        </div>
                    </div>

                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
                        <h3 class="font-bold text-lg text-gray-900 dark:text-white mb-4 flex items-center gap-2"><svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <form action="#" method="POST" class="space-y-4">
                            ...
                            <button type="submit"
                                class="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white py-3 rounded-lg font-semibold shadow-md transition-all duration-200">Subscribe</button>
                        </form>
                    </div>
                </aside>
            </div>
            ...
            <section class="mt-12">
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ...
                        @foreach($posts->skip(1) as $post)
                            <article
                                ...
                                <div class="h-40 bg-cover bg-center"
                                    style="background-image: url('{{ $post->image ?? asset('images/placeholder.jpg') }}')">
                                </div>
                                <div class="p-6">
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mb-2">
                                        {{ optional($post->created_at)->format('M d, Y') }} •
                                        {{ optional($post->user)->name ?? 'Staff' }}</div>
                                    <h4 class="font-semibold text-xl text-gray-900 dark:text-white mb-3 leading-tight">
                                        <a href="{{ route('posts.show', $post) }}"
                                            class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200">{{ $post->title }}</a>
                                    </h4>
                    ...
                    @else
                        <div
                            class="md:col-span-2 lg:col-span-3 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg text-center">
