        ...
        <div class="max-w-7xl mx-auto px-6">
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($posts as $post)
                        <article
                            ...
                            @if($post->image)
                                <div class="h-48 bg-cover bg-center" style="background-image: url('{{ asset($post->image) }}')">
                                </div>
                            @endif
                            <div class="p-6">
                                ...
                                <div class="flex items-center justify-between">
                                    @if($post->category)
                                        <span
                                            class="inline-block px-3 py-1 text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full">
                                            {{ $post->category->name }}
                                        </span>
                                    @endif
                                    <a href="{{ route('public.posts.show', $post) }}"
                                        class="text-blue-600 dark:text-blue-400 hover:underline font-medium text-sm">Read
                                        More</a>
                                </div>
                            </div>
                        </article>
            ...
            @else
                <div class="text-center py-12">
                    <p class="text-gray-500 dark:text-gray-400 text-lg">No published posts available in this category. <a
                            href="{{ route('public.posts.index') }}" class="text-blue-600 hover:underline">View all
                            posts</a>.</p>
                </div>
            @endif
        </div>
